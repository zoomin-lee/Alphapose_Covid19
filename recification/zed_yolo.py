...
def Reprojection3D(image, disparity, f, b):
    Q = np.array([[1, 0, 0, -1100 / 2], [0, 1, 0, -608 / 2], [0, 0, 0, f], [0, 0, -1 / b, -124.343 / b]])
...
def init_calibration(calibration_file, image_size) :
    ...
    return cameraMatrix_left, cameraMatrix_right, map_left_x, map_left_y, map_right_x, map_right_y, left_cam_fx, T_[0]
...
def main(argv):
    ...
    camera_matrix_left, camera_matrix_right, map_left_x, map_left_y, map_right_x, map_right_y, fx, baseline = init_calibration(calibration_file, image_size)
    ...
    while key != 113:  # for 'q' key
        ...
        # kernel_size=3
        # left_rect = cv2.GaussianBlur(left_rect, (kernel_size,kernel_size), 1.5)
        # right_rect = cv2.GaussianBlur(right_rect, (kernel_size, kernel_size), 1.5)
        ...
        filtered = (filtered / 16).astype(np.uint64)
        ...
        depth = np.zeros(shape=left_rect.shape).astype(float)
        depth[filtered > 0] = (fx * baseline) / (units * filtered[filtered > 0])

        # depth = Reprojection3D(left_rect, filteredImg, left_cam_fx, baseline)

        # Do the detection
        detections = detect(netMain, metaMain, left_rect, thresh)
        ...
        for detection in detections:
            ...
            pstring = label+": "+str(np.rint(100 * confidence))+"%q"
            log.info(pstring)
            bounds = detection[2]
            ...
            distance = 0.25*(depth[bounds[0], bounds[1]] + depth[bounds[0]+5, bounds[1]+5] +depth[bounds[0]-5, bounds[1]-5] +depth[bounds[0]+5, bounds[1]-5])
            cv2.rectangle(left_rect, (x_coord - thickness, y_coord - thickness),
                          (x_coord + x_extent + thickness, y_coord + (18 + thickness*4)),
                          color_array[detection[3]], -1)
            cv2.putText(left_rect, label + " " +  (str(distance) + " mm"),
